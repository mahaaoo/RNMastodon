...
const FollowButton: React.FC<FollowButtonProps> = (props) => {
  ...
  const { data: followData, run: getFollowById } = useRequest(fetchFollowById(id), { manual: true, loading: false });
  const { data: unFollowData, run: getUnfollowById } = useRequest(fetchUnfollowById(id), { manual: true, loading: false });
  ...
  useEffect(() => {
    if(relationships && relationships.length > 0) {
      const relationship = relationships.filter(item => item.id === id)[0];
