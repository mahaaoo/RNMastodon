...
import { StyleSheet, Text, View, Animated, TouchableOpacity, ViewStyle, TextStyle } from "react-native";
...
const MyTabBarItem: React.FC<MyTabBarItemProps> = (props) => {
  const { activeTextColor, inactiveTextColor, textStyle, isTabActive, name, activeTab=0, onPressHandler, tabStyle } = props;
  ...
  return (
    ...
    >
      <View style={[styles.tab, tabStyle, ]}>
        <Text style={[{color: textColor, fontWeight, fontSize: 17 }, textStyle ]}>
          {name}
        </Text>
      </View>
    </TouchableOpacity>
...
const MyTabBar: React.FC<MyTabBarProps> = (props) => {
  const { containerWidth = Screen.width, tabs, scrollValue, backgroundColor, style, activeTab=0, goToPage, tabBarUnderlineStyle } = props;
  ...
  return (
    <View style={[styles.tabs, {backgroundColor: backgroundColor, }, style ]}>
      {tabs?.map((name, page) => {
        ...
        return <MyTabBarItem key={`MyTabBarItem${page}`} name={name} activeTab={page} isTabActive={isTabActive} onPressHandler={goToPage} />
      })}
      <Animated.View
        style={[
          //@ts-ignore
